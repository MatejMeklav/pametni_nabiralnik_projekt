...
class Oglas {
  ...
  //konstruktor
  public function __construct($id, $naslov, $vsebina,$datumObjave, $user_id) {
    $this->id      = $id;
    $this->naslov  = $naslov;
...
//v zanki ustvarjamo nove objekte in jih dajemo v seznam
    while($row = mysqli_fetch_assoc($result)){
      $list[] = new Oglas($row['ID'], $row['Naslov'], $row['Vsebina'],$row['DatumObjave'],$row['user_id']);
    }
  ...
  public static function najdi($id) {
    ...
    return new Oglas($row['ID'], $row['Naslov'], $row['Vsebina'],$row['DatumObjave'],$row['user_id']);
  }

    public static function edit($naslov, $vsebina, $id){

        $query="UPDATE oglas SET Naslov='$naslov', Vsebina='$vsebina' WHERE id='$id';";
        $db = Db::getInstance();
  ...
  public static function dodaj($naslov,$vsebina, $user_id) {
    
    $db = Db::getInstance();
    
	  //primer query-a s prepared statementom

    if ($stmt = mysqli_prepare($db, "Insert into Oglas (Naslov,Vsebina,DatumObjave, user_id) Values (?,?,now(),?)")) {
			//dodamo parametre po vrsti namesto vprašajev
			//s string, i integer ,d double, b blob
     mysqli_stmt_bind_param($stmt, "ssi",$naslov,$vsebina, $user_id);
